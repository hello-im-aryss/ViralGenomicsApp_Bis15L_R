...
# This package was giving me problems with the most recent version of R so I used RColorBrewer instead.
```{r}
#colors <- LaCroixColoR::lacroix_palette("Pamplemousse", type = "discrete")
```

# I separated out your summary here
```{r}
dat <- arch_bact %>% 
  group_by(host) %>% 
  summarize(size = mean(size_mb, na.rm = T))
...
# I then added the RColorBrewer palette "Paired". The problem was, as I expected, that we didn't have the fill in the original ggplot call.
```{r}
dat %>% 
